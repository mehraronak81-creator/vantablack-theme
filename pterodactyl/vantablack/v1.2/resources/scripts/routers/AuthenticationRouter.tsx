...
const TopBar = () => {
    const { t } = useTranslation('vantablack/auth');
    const [guildData, setGuildData] = useState<{ instant_invite: string } | null>(null);

    const name = useStoreState((state: ApplicationStore) => state.settings.data!.name);
    const logo = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.logo);
    const logoHeight = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.logoHeight);
    const fullLogo = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.fullLogo);
    const socialPosition = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.socialPosition);
    const logoPosition = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.logoPosition);
...
const AuthContainer = () => {
    const loginBackground = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.loginBackground);
    const loginLayout = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.loginLayout);
    const loginGradient = useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.loginGradient);
    ...
    return (
        <div className={'min-h-screen h-full bg-center bg-no-repeat bg-cover z-10 relative'} css={`background-image:url(${loginLayout == 1 ? loginBackground : ''});`}>
            ...
            <div className={'grid lg:grid-cols-2'}>
                ...
                <div className={'h-full bg-center bg-no-repeat bg-cover bg-vantablack z-10 relative'} css={`background-image:url(${loginBackground});`}>
                    <Gradient />
                </div>
            </div>
            : 
            <div className={'grid lg:grid-cols-2'}>
                ...
                <div className={'h-full lg:p-5'}>
                    <div className={'h-full bg-center bg-no-repeat bg-cover bg-vantablack rounded-box overflow-hidden relative z-10'} css={`background-image:url(${loginBackground});`}>
                        <Gradient />
                    </div>
                </div>
            </div>
