...
export default ({ activity, children }: Props) => {
    ...
    const ipFlag = String(useStoreState((state: ApplicationStore) => state.settings.data!.vantablack.ipFlag));

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://api.country.is/${activity.ip}?v=3041cf234d50072cfa636ac560ac966f`);
        ...
        <div className={'grid relative grid-cols-10 py-5 mb-2 last:mb-0 bg-gray-700 backdrop rounded-box'}>
            <div className={'hidden sm:flex sm:col-span-1 items-center justify-center select-none'}>
                <div className={'flex items-center w-10 h-10 rounded-full bg-gray-600 overflow-hidden'}>
                    <UserAvatar uuid={actor?.uuid || '%%__NONCE__%%'} email={actor?.email || '%%__NONCE__%%'} user={actor?.username || '%%__NONCE__%%'} width={'40px'} />
                </div>
            </div>
                ...
                <div className={'flex-1 px-4 sm:px-0'}>
                    <div className={'flex items-center text-gray-50'}>
                        ...
                        <Link
                            ...
                            className={'transition-colors duration-75 active:text-vantablack hover:text-vantablack'}
                        >
                            {activity.event}
                        </Link>
